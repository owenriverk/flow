// The Grand Canyon of the Stikine beside the oracle: a real map (kilometres on
// both axes, 40 m contours, the named rapids, the Tanzilla coming in at the
// Slot) that the page scrolls down. The blue line draws itself as you read and
// an orange boat rides its tip, so the boat is where you are in the report.
// Data + contours come from owenkurth/scripts/build-stikine-topo.mjs.
import { FRAME, MARKS, NORTH, STIKINE, TANZILLA, TOPO } from "./stikine-data.js";

const SVG_NS = "http://www.w3.org/2000/svg";
const host = document.querySelector("[data-stikine-river]");
...
if (host && box && svg && topoSvg && topo && flow && bed && trib && marks && north && boat) {
  ...
  const build = () => {
    ...
    const d = smoothPath(STIKINE.map(([x, y]) => [X(x), Y(y)]));
    flow.setAttribute("d", d);
    bed.setAttribute("d", d);
    trib.setAttribute("d", smoothPath(TANZILLA.map(([x, y]) => [X(x), Y(y)])));
